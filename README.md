...
| `request_takeoff` | `{"drone_id": str}` | `{"battery": float, "port_id": str, "port_coordinates": {"lat": str, "lon": str}, "from": "drone_manager"}` или `{"error": str, "from": "drone_manager"}` | Освободить порт и отправить событие старта взлета в SITL |
